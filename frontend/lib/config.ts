...
export const config = {
  ...
  apiBaseUrl: isServer
    ? (process.env.INTERNAL_API_URL || process.env.NEXT_PUBLIC_API_BASE_URL || "http://backend:8000/api/v1")
    : (process.env.NEXT_PUBLIC_API_BASE_URL || "/api/v1"),

  wsBaseUrl: isServer
    ? (process.env.INTERNAL_WS_URL || process.env.NEXT_PUBLIC_WS_BASE_URL || "ws://backend:8000/api/v1/ws")
